...
//    private var paymentModelList:[PaymentModel] = []
    private var displayedPaymentList: [PaymentModel] = []
    ...
    func sumPayment(_:Date) -> Int{
        ...
        let dayCheck = displayedPaymentList.filter({$0.date >= (firstDay)})
    ...
    func sumDayPayment(_: Date) -> Int{
        let sum = displayedPaymentList.filter{$0.date.zeroclock == selectedDate.zeroclock}.map{$0.price}.reduce(0){$0 + $1}
        return sum
    }
    ...
    func setTableView(_: Date) {
        ...
        let realm = try! Realm()
        let paymentsList = realm.objects(PaymentModel.self).sorted(byKeyPath: "date",ascending: false)
        paymentsList.forEach{payment in
            if payment.date.zeroclock == selectedDate.zeroclock{
                result.append(payment)
            }
        }
        
        displayedPaymentList = result
        householdAccountBookTableView.reloadData()
        
        print("hkr dataSource: \(self.displayedPaymentList.count)")
    }
...
extension CalendarViewController:UITableViewDelegate,UITableViewDataSource{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView.tag == 0 {
            return displayedPaymentList.count
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView.tag == 0{
            ...
            let item = displayedPaymentList[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if tableView.tag == 0{
            let target = displayedPaymentList[indexPath.row]
            let realm = try! Realm()
            if !target.isInvalidated{
                displayedPaymentList.remove(at: indexPath.row)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView.tag == 0{
            ...
            inputViewController.payment? = displayedPaymentList[indexPath.row]
            inputViewController.setPaymentData(data: displayedPaymentList[indexPath.row])
...
extension CalendarViewController:FSCalendarDataSource,FSCalendarDelegate,FSCalendarDelegateAppearance{
    func calendar(_ calendar: FSCalendar, numberOfEventsFor date: Date) -> Int {
        let dateList = displayedPaymentList.map({$0.date.zeroclock})
